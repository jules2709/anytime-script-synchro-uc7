...
class SyncZendeskMessagesHandler
{
    ...
    public function __invoke(SyncZendeskMessagesCommand $command): void
    {
        ...
        while ($hasNextPage) {
            ...
            if (!empty($commentsByTicket)) {
                ...
                if (!empty($filteredTicketIds)) {
                    echo "⏭️  Tickets ignorés (tag historique_admin) : " . count($filteredTicketIds) . "\n";
        ...
        echo "⏭️  Tickets filtrés (Historique_Admin) : {$filteredTicketsCount}\n";
        echo "⚠️  Tickets manquants : {$missingTicketsCount}\n";
